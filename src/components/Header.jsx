...
export default function Header({ user, panel, setPanel, onHome, onLogout, onProfile }) {
  ...
  const friendBadge = (panel.friendRequests || []).filter((request) => request.recipientId === user.id).length;
  ...
  return (
    <header className="app-header">
      ...
      <div className="header-actions">
        ...
        <IconButton label="Match inbox" className="mail-button" badge={proposalsBadge + friendBadge} onClick={() => setPanel(panel.open === "mail" ? "" : "mail")}>
          <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
